...
class notesCog(commands.Cog):
    ...
    async def addnote(self, ctx, *, note: str) -> None:
        ...
        if not conv:
            await ctx.send(embed=common_embed(title='Missing Permissions',
                                              description="You aren't in a valid modmail channel, if this is incorrect please contact my makers"))
            return
    ...
    async def notes(self, ctx, user: typing.Optional[typing.Union[discord.Member, str]]) -> None:
        if user is None:
            ...
            if not result:  # Sorry for this spaghetti code
                await ctx.send(
                    embed=common_embed(title='Not Found', description="Unable to locate modmail thread, please specify the id"))
                return
            ...
            try:
                ...
                await ctx.send(embed=common_embed(title='Not Found', description="Unable to locate user, please check if the id is correct"))
                return
    ...
    async def editnote(self, ctx, note_id: int, *, new_text: str) -> None:
        ...
        if results[0] != ctx.author.id:
            await ctx.send(common_embed(title="Missing Permissions", description="You do not have access to edit this note, or the note doesn't exist"))
            return
    ...
    async def editnote_error(self, ctx, err: any) -> None:
        if isinstance(err, commands.CheckFailure):
            ...
            await ctx.send("This argument is not valid, please check if the id or text is correct")
        else:
            await ctx.send(f"Unknown error occurred.\n{str(err)}")
    ...
    async def note(self, ctx, note_id: int) -> None:
        results = await self.db_conn.fetchrow("SELECT id, user_id, made_by_id, note FROM modmail.notes WHERE id = $1",
                                              note_id)
        if not results:
            await ctx.send(embed=common_embed(title="Not Found", description="Unable to locate note, please check the id and try again"))
    ...
    async def note_error(self, ctx, err: any):
        if isinstance(err, commands.CheckFailure):
            ...
            await ctx.send("This argument is not valid, please check if the id is correct")
        else:
            await ctx.send(f"Unknown error occurred.\n{str(err)}")
    ...
    async def deletenote(self, ctx, note_id: int):
        results = await self.db_conn.fetchrow("SELECT made_by_id FROM modmail.notes WHERE id = $1", note_id)
        if results[0] != ctx.author.id:
            await ctx.send(embed=common_embed(title="Not found", description="You do not have access to delete this note, or the note doesn't exist"))
            return

        try:
            await self.db_conn.execute("DELETE FROM modmail.notes WHERE id = $1", note_id)
        finally:
            await ctx.send(embed=common_embed(title="Success", description='Successfully deleted note with id: `{note_id}`'))
