...
# Reply takes bot commands.Bot, ctx commands.Context, db_conn asyncpg.Pool, user_id int, message str, conv_id int and anon bool default false
#  if anon true => Replies to user anonymously
#  else replies to user
#  returns nothing on success, error on failure
async def reply(bot: commands.Bot, ctx: commands.Context, db_conn: Pool, user_id: int, message: str, conv_id: int,
    ...
    thread_embed.set_footer(text=ctx.author.roles[-1].name if anon else 'anonymous reply')
